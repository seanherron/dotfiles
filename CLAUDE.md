...
The split is driven by two data axes: `{{ "{{" }} .chezmoi.os {{ "}}" }}` (`"darwin"` vs `"linux"`) and `{{ "{{" }} .profile {{ "}}" }}` (`"personal"` vs `"work"`):

- **`.chezmoiignore`** is a template with two gated blocks. The non-darwin block ignores the macOS-only artifacts (`Library` editor settings, `.config/ghostty`, `.config/cmux`, the font/extension installers, `run_onchange_macos-defaults.sh`, and `setup-sudo-auth.sh`), so they never deploy to Linux. The non-work block ignores `setup-sudo-auth.sh` again, so the sudo-auth hook only lands when the machine is **both** darwin **and** work. (NB: for `run_` scripts the ignore matches the *target* name — the source name minus the `run_onchange_` prefix.)
...
- **`run_onchange_install-packages.sh.tmpl`** branches on OS: macOS installs `brews` + `darwinBrews` + `casks` + `mas`, then installs `darwinNpmGlobals` via npm (on the personal profile it keeps a sudo credential alive across a long `brew bundle`; work needs no sudo since its casks are all CLI/user-owned); Linux bootstraps Homebrew-on-Linux if missing, then installs the cross-platform `brews` only. It also branches on the **`.profile`** data value (`personal` vs `work`): `personalCasks` are only installed when `profile == "personal"`, so work machines skip them. These are GUI apps that on a work machine are provided (root-owned) by IT/MDM and self-update — `1password`, `google-chrome`, `visual-studio-code`, `cursor`, `claude`, `chatgpt`, `codex-app`. Letting `brew bundle` manage them on work just causes upgrade churn, and each upgrade of a root-owned app needs sudo (the cause of repeated password prompts during `chezmoi apply`). CLI casks such as `claude-code` and `1password-cli` install user-owned and stay in cross-profile `casks`; npm-distributed macOS CLIs such as `codex` belong in `darwinNpmGlobals`. Conversely, `workBrews` install only when `profile == "work"` (currently `pam-u2f` + `pam-reattach`, feeding the sudo-auth hook).
- **`run_onchange_setup-sudo-auth.sh.tmpl`** (work + darwin only) writes `/etc/pam.d/sudo_local` with optional/sufficient auth lines for Touch ID (`pam_tid`), `pam_reattach` (makes Touch ID work inside tmux/zellij/cmux; `ignore_ssh`), and a FIDO2 security key (`pam_u2f`). All lines are optional/sufficient and the password stack still runs last, so there's no lockout risk; it's idempotent and only `sudo tee`s the file when content differs.
- **`run_onchange_install-ghostty-terminfo.sh.tmpl`** deploys everywhere but self-guards with a `{{ "{{" }} if ne .chezmoi.os "darwin" {{ "}}" }}` wrapper so it only does work on Linux: it fetches the pre-compiled `xterm-ghostty` terminfo into `~/.terminfo` so Ghostty works over SSH (macOS gets it from the app bundle). Failures warn but don't abort.
- **`private_dot_ssh/private_config.tmpl`** gates the Apple-only `UseKeychain` directive behind darwin (it errors out on Linux ssh), and gates an `Include ~/.ssh/devbox/*` line behind non-personal (i.e. work) profile — work machines keep per-host devbox configs out of this public repo.
...
When adding a file, decide which profile(s) it belongs to: macOS-only → add it to the `.chezmoiignore` non-darwin block; work-only → add it to the non-work block; cross-platform → leave it (it deploys everywhere); needs to differ → make it a `.tmpl` and branch on `.chezmoi.os` / `.profile`.
...
| `.chezmoidata/packages.yaml` | — | Data file: `brews` (cross-platform), `darwinBrews` + `darwinNpmGlobals` (macOS-only), `workBrews` (macOS + `profile == "work"`), `casks` + `mas` (macOS-only), `personalCasks` (macOS + `profile == "personal"`) |
| `run_onchange_install-packages.sh.tmpl` | — | Script: runs `brew bundle` and macOS npm global installs whenever `packages.yaml` changes; branches macOS vs Linux and on `.profile` |
| `run_onchange_setup-sudo-auth.sh.tmpl` | `/etc/pam.d/sudo_local` | Work + darwin only: Touch ID / security-key / password sudo auth |
| `run_onchange_install-ghostty-terminfo.sh.tmpl` | `~/.terminfo/x/xterm-ghostty` | Linux only (self-guarded): install `xterm-ghostty` terminfo for SSH |

## Template system

`run_onchange_install-packages.sh.tmpl` is a Go template that reads from `.chezmoidata/packages.yaml`. To add a new package, edit `packages.yaml`: put cross-platform Homebrew CLI tools under `brews` (installed on both OSes), macOS-only npm-installed global CLIs under `darwinNpmGlobals`, macOS-only formulae under `darwinBrews`, macOS work-machine-only formulae under `workBrews` (installed only when `profile == "work"`), GUI apps under `casks`, and GUI apps that are provided by IT/MDM on work machines (they install root-owned and self-update) under `personalCasks` (installed only when `profile == "personal"`, so brew doesn't fight MDM or trigger sudo prompts on work). The install script re-runs automatically on the next `chezmoi apply` because chezmoi hashes the rendered output to detect changes.
