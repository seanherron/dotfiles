...
| `.chezmoidata/packages.yaml` | — | Data file: `brews` (cross-platform), `darwinBrews` (macOS-only), `workBrews` (macOS + `profile == "work"`), `casks` + `mas` (macOS-only), `personalCasks` (macOS + `profile == "personal"`) |
| `run_onchange_install-packages.sh.tmpl` | — | Script: runs `brew bundle` whenever `packages.yaml` changes; branches macOS vs Linux and on `.profile` |
